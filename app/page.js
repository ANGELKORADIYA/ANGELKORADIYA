...
import { HackerHudProvider } from "./components/HackerHudContext";
import HackerHUD from "./components/HackerHUD";
import { useHackerHud } from "./components/HackerHudContext";
...
function AcademicItem({ edu, terminalRef }) {
  const { showHud, hideHud } = useHackerHud();
  const itemRef = useRef(null);

  const handleMouseEnter = useCallback(() => {
    if (itemRef.current) {
      showHud(itemRef.current, { type: "academic", ...edu });
    }
    // Glitch animation
    if (itemRef.current) {
      itemRef.current.classList.remove("hud-glitch");
      void itemRef.current.offsetWidth;
      itemRef.current.classList.add("hud-glitch");
    }
  }, [edu, showHud]);
  ...
  return (
    <div
      ref={itemRef}
      onClick={() =>
        terminalRef.current?.execute(`inspect --academic ${edu.id}`, {
          type: "academic",
          ...edu,
        })
      }
      ...
      className="relative pl-12 group/item cursor-pointer hud-card-hover"
      role="button"
      tabIndex={0}
    >
      {/* Corner brackets */}
      ...
      <span className="text-[10px] font-black text-sky-600 dark:text-sky-400 uppercase tracking-widest group-hover/item:text-sky-500 dark:group-hover/item:text-sky-300 transition-colors">
        {edu.year}
      </span>
      <h3 className="text-lg font-bold text-gray-900 dark:text-white mt-1 group-hover/item:text-sky-600 dark:group-hover/item:text-sky-300 transition-colors">
        {edu.title}
      </h3>
      <p className="text-gray-500 dark:text-gray-400 text-sm group-hover/item:text-gray-600 dark:group-hover/item:text-gray-300 transition-colors mb-2">
        {edu.place}
      </p>
      <p className="text-xs text-gray-600 dark:text-gray-300 font-medium bg-gray-100 dark:bg-white/10 inline-block px-3 py-1.5 rounded-lg border border-gray-200 dark:border-white/10 group-hover/item:border-sky-500/30 group-hover/item:bg-sky-500/5 dark:group-hover/item:bg-sky-500/10 transition-colors">
        {edu.detail}
      </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-8 p-8 rounded-[2rem] border border-gray-200 dark:border-white/10 bg-white/50 dark:bg-white/5 backdrop-blur-[20px] saturate-[180%] hover:bg-white/80 dark:hover:bg-white/10 hover:border-sky-500/30 transition-all duration-500 relative group overflow-hidden">
                  ...
                  <h2 className="text-2xl font-extrabold text-gray-900 dark:text-white mb-8 flex items-center gap-3 tracking-wide uppercase">
                    <span className="w-10 h-10 bg-sky-500/10 dark:bg-sky-500/20 text-sky-600 dark:text-sky-400 rounded-xl flex items-center justify-center text-lg border border-sky-200 dark:border-sky-500/30">
                      🎓
                    </span>
                    <span className="bg-clip-text text-transparent bg-gradient-to-r from-sky-600 to-indigo-600 dark:from-sky-400 dark:to-indigo-400">
                ...
                <div className="md:col-span-4 flex flex-col gap-6">
                  ...
                  <div className="flex-1 p-6 rounded-[2rem] border border-gray-200 dark:border-white/10 bg-white/50 dark:bg-white/5 backdrop-blur-[20px] saturate-[180%] hover:bg-white/80 dark:hover:bg-white/10 hover:border-amber-500/30 transition-all duration-500 group relative overflow-hidden">
                    ...
                    <h2 className="text-lg font-extrabold text-gray-900 dark:text-white mb-6 flex items-center gap-3 tracking-wide uppercase">
                      <span className="w-8 h-8 bg-amber-500/10 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 rounded-lg flex items-center justify-center text-base border border-amber-200 dark:border-amber-500/30">
                        🏆
                      </span>
                    ...
                    <div className="space-y-3">
                      <div className="p-4 bg-gray-50/50 dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/5 group-hover:border-amber-500/20 transition-colors">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          Qualified in the first attempt.
                        </p>
                      </div>
                      <div className="p-4 bg-gray-50/50 dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/5 group-hover:border-sky-500/20 transition-colors">
                        ...
                        <a
                          ...
                          className="text-[10px] font-black text-sky-600 dark:text-sky-400 uppercase tracking-widest hover:text-sky-700 dark:hover:text-sky-300"
                        >
                          View Profile →
                        </a>
                      </div>
                  ...
                  <div className="p-6 rounded-[2rem] border border-gray-200 dark:border-white/10 bg-white/50 dark:bg-white/5 backdrop-blur-[20px] saturate-[180%] hover:bg-white/80 dark:hover:bg-white/10 transition-all duration-500">
                    <h2 className="text-[10px] font-extrabold text-gray-400 uppercase tracking-wide mb-4 text-center">
                      {"{"}Interests{"}"}
                    </h2>
                    <div className="flex justify-center gap-6">
                      {["🎮", "✈️", "💻"].map((emoji, i) => (
                        <div
                          key={i}
                          className="text-3xl hover:scale-125 hover:-translate-y-2 transition-transform cursor-none"
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-4 p-3 rounded-2xl bg-gray-50/50 dark:bg-white/5 border border-gray-100 dark:border-white/5 hover:bg-gray-100/80 dark:hover:bg-white/10 hover:border-emerald-500/30 transition-all group/link"
                      >
                        ...
                        <span className="font-bold text-gray-700 dark:text-gray-300 group-hover/link:text-emerald-600 dark:group-hover/link:text-emerald-400 transition-colors">
                          {resource.name}
                        </span>
                        <span className="ml-auto text-emerald-600 dark:text-emerald-400 opacity-0 group-hover/link:opacity-100 group-hover/link:translate-x-1 transition-all">
                          →
                        </span>
                      </a>
                    ))}
                ...
                <div className="md:col-span-6 p-8 rounded-[2rem] border border-sky-200 dark:border-sky-500/30 bg-sky-50 dark:bg-sky-500/10 backdrop-blur-[20px] saturate-[180%] flex flex-col items-center justify-center text-center relative overflow-hidden group">
                  ...
                  <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-4 tracking-wide uppercase">
                    {"{"}Start a Project{"}"}
                  </h2>
                  <p className="text-sky-700 dark:text-sky-200 mb-8 max-w-sm">
                    My inbox is always open. Let&apos;s build something amazing
                    together.
                  </p>
                  <button
